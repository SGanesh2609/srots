...
export const JobDetailView: React.FC<JobDetailViewProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'rounds' | 'applicants' | 'eligible'>(() => {
        const t = searchParams.get('dtab') as 'overview' | 'rounds' | 'applicants' | 'eligible';
        return ['overview', 'rounds', 'applicants', 'eligible'].includes(t) ? t : 'overview';
    });

    const handleTabChange = (tab: 'overview' | 'rounds' | 'applicants' | 'eligible') => {
        setActiveTab(tab);
        setSearchParams(prev => { const p = new URLSearchParams(prev); if (tab !== 'overview') p.set('dtab', tab); else p.delete('dtab'); return p; }, { replace: true });
    };
    ...
    // Safe accessors with fallback to deprecated field names (all properly typed in Job interface)
    const companyName     = job.companyName     || job.company         || 'Unknown Company';
    const jobTypeDisplay  = job.jobType         || job.type            || 'Full-Time';
    const workModeDisplay = job.workMode        || job.workArrangement || 'On-Site';
    const applicantCount  = job.applicantCount  ?? (job.applicants?.length ?? 0);

    /**
     * Download handler.
     * 'applicants'     → GET /jobs/{id}/export-list?type=applicants
     * 'not-interested' → GET /jobs/{id}/export-list?type=not-interested
     */
    const handleDownload = async (type: 'applicants' | 'not-interested') => {
        try {
            await JobService.exportJobList(job.id, type);
        } catch (err: any) {
