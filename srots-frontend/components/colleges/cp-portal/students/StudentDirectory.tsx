...
export const StudentDirectory: React.FC<StudentDirectoryProps> = ({ user }) => {
  ...
  const [activeTab, setActiveTab] = useState<'directory' | 'accounts' | 'specifications'>(() => {
    const t = searchParams.get('tab') as 'directory' | 'accounts' | 'specifications';
    return ['directory', 'accounts', 'specifications'].includes(t) ? t : 'directory';
  });

  const handleTabChange = (tab: 'directory' | 'accounts' | 'specifications') => {
    setActiveTab(tab);
    setSearchParams(prev => { const p = new URLSearchParams(prev); if (tab !== 'directory') p.set('tab', tab); else p.delete('tab'); return p; }, { replace: true });
  };
