...
/**
 * ProfileTab.tsx
 * Path: src/components/student/profile/tabs/ProfileTab.tsx
 *
 * ─── FIXES IN THIS VERSION ────────────────────────────────────────────────────
 *
 * 1. FIX — "onUpdate is not a function" crash:
 *    ROOT CAUSE: Parent component (StudentProfile.tsx) passes the OLD prop API:
 *      <ProfileTab onUpdateProfile={fn} onUpdateUser={fn} />
 *    The new ProfileTab expected only `onUpdate` → undefined → crash on save.
 *
 *    FIX: Props now accept ALL THREE, all optional with safe defaults:
 *      onUpdate?:        (student: Student) => void   ← new unified callback
 *      onUpdateProfile?: (updates) => void            ← legacy compat
 *      onUpdateUser?:    (user) => void               ← legacy compat
 *
 *    propagateUpdate() calls whichever callbacks the parent provided.
 *    If the parent provides none of them, saves still work and toast shows —
 *    the page just won't refresh state (which would be a parent bug to fix).
 *
 * 2. FIX — linkedInProfile always blank after save:
 *    ROOT CAUSE: Java field `linkedinProfile` → Jackson → `linkedinProfile` (lowercase n).
 *    Frontend type expects `linkedInProfile` (capital I).
 *    mapStudent360ToStudent() now normalises this (in studentService.ts).
 *    ProfileTab reads `localData.linkedInProfile` which now has the correct value.
 *
 * 3. Address saving — how it works:
 *    saveAddresses() sends two sequential PUT requests:
 *      PUT /students/profile/address/current   { addressLine1, addressLine2,
 *      PUT /students/profile/address/permanent    village, mandal, city,
 *                                                 state, zip, country }
 *    Both match AddressFormData and the backend AddressRequest DTO exactly.
 *    The second response (permanent) is used to propagate the full update.
 *
 * 4. SECURITY — what localData is and what students can edit:
 *    `localData` is React component state (in-memory, NOT localStorage/cookies).
 *    It cannot be tampered with via DevTools to bypass server-side validation.
 *
 *    Student-editable sections: Contact Info, Education Gaps, More Info, Addresses.
 *    Locked sections (displayed read-only, no edit button):
 *      rollNumber, branch, course, batch, fullName, aadhaarNumber, dob, gender,
 *      nationality, religion, placementCycle, instituteEmail, phone, whatsappNumber,
 *      parentPhone, parentEmail, mentor, advisor, coordinator.
 *
 *    Backend enforcement: PATCH /students/profile/self only updates the
 *    StudentSelfUpdateRequest fields. Any other fields in the JSON body are
 *    ignored by Jackson (unknown property handling). Students cannot edit locked
 *    fields even by sending a crafted request body, because:
 *      a) Those fields are not in StudentSelfUpdateRequest.
 *      b) The studentId in the endpoint always comes from the JWT — never the body.
 */
