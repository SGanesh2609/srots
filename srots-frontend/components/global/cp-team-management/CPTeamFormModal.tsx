...
/**
 * Component: CPTeamFormModal
 * Path: components/global/cp-team-management/CPTeamFormModal.tsx
 *
 * Unified create/edit modal for CP team members (CPH and STAFF).
 *
 * Used by:
 *   CPTeamManagement (mode='admin')  → showRoleSelector=true  → can create CPH or STAFF
 *   CPTeamManagement (mode='cph')    → showRoleSelector=false → always creates STAFF
 */

// ── Types ─────────────────────────────────────────────────────────────────────

export interface CPTeamFormData {
    ...
    role:           Role.CPH | Role.STAFF;
    address:        AddressFormData;
}

interface CPTeamFormModalProps {
    isOpen:           boolean;
    onClose:          () => void;
    onSave:           (data: CPTeamFormData, userId?: string) => void;
    initialData?:     User | null;
    collegeCode:      string;
    collegeName?:     string;
    /** true = Admin/Dev portal (shows CPH/STAFF role selector)
     *  false = CPH portal (role locked to STAFF) */
...
export const CPTeamFormModal: React.FC<CPTeamFormModalProps> = ({
    isOpen, onClose, onSave, initialData, collegeCode, collegeName, showRoleSelector,
}) => {
    ...
    const usernamePreview = () => {
        const effectiveRole = showRoleSelector ? role : Role.STAFF;
        const typeStr = effectiveRole === Role.CPH ? 'CPADMIN' : 'CPSTAFF';
        const suffix  = usernameSuffix.trim() ||
    ...
    const prefixLabel = () => {
        const effectiveRole = showRoleSelector ? role : Role.STAFF;
        const typeStr = effectiveRole === Role.CPH ? 'CPADMIN' : 'CPSTAFF';
        return `${collegeCode}_${typeStr}_`;
    };
                ...
                {!isEdit && (
                    <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 flex gap-2">
                        ...
                        <p className="text-xs text-blue-700">
                            {showRoleSelector
                                ? 'Password is auto-generated from the username + Aadhaar digits and emailed to the user on account creation.'
                                : <>Role is <strong>Placement Staff</strong>. Password is auto-generated from the username + Aadhaar digits and emailed on account creation.</>
                            }
                        </p>
                    </div>
                )}
