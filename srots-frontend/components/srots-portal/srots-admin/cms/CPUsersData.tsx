...
import { CollegeService } from '../../../../services/collegeService';
import { CPUsersHeader } from './CPUsersHeader';
import { CPUsersTable, SoftHardDeleteModal } from './CPUsersTable';
import { CPUserFormModal } from './CPUserFormModal';
import { BulkUploadResultModal } from '../../../common/BulkUploadResultModal';
import { Users, Archive } from 'lucide-react';

/**
 * Component: CPUsersData
 * Path: components/colleges/cms/CPUsersData.tsx
 *
 * Endpoints used:
 *   GET    /accounts/college/{id}/role/CPH|STAFF?status=active|soft_deleted
 ...
 *   PATCH  /accounts/{userId}/restrict
...
const ResendConfirmModal: React.FC<{
  user: User | null; onClose: () => void; onConfirm: () => void; sending: boolean;
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-gray-100 p-6 space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            New login credentials will be sent to{' '}
...
// ── Main ──────────────────────────────────────────────────────────────────────
export const CPUsersData: React.FC<CPUsersDataProps> = ({
  collegeId, collegeCode, collegeName, onRefresh,
}) => {
  ...
  // Tab counts — fetched independently so switching tabs shows live numbers
  ...
  // Restore
  const [isRestoring,   setIsRestoring]   = useState<string | null>(null);  // userId
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      console.error('[CPUsersData] fetch failed', err);
  ...
  // ── Fetch tab counts (both tabs, no search filter) ─────────────────────────
  const fetchCounts = useCallback(async () => {
    ...
    } catch { /* counts are decorative — silent fail is fine */ }
  }, [collegeId]);
  ...
  const handleSave = async (data: Partial<User> & Record<string, any>, userId?: string) => {
    try {
      if (userId) {
        await CollegeService.updateCPUser(collegeId, userId, data);
      } else {
        await CollegeService.createCPUser(collegeId, data);
      }
  ...
  // Delete — called from SoftHardDeleteModal with the chosen mode
  ...
  return (
    <div className="space-y-5 animate-in fade-in">
      ...
      <CPUsersHeader
        ...
        onAddUser={() => { setEditUser(null); setShowFormModal(true); }}
        onBulkUpload={handleBulkUpload}
        onDownloadTemplate={() => CollegeService.downloadCPUsersTemplate()}
      />

      {/* ── Status filter tabs ───────────────────────────────────────────── */}
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
        {([
          { value: 'active'       as StatusFilter, label: 'Active',       Icon: Users,   count: activeCount,      color: 'text-green-600'  },
          { value: 'soft_deleted' as StatusFilter, label: 'Soft Deleted', Icon: Archive, count: softDeletedCount, color: 'text-amber-600'  },
        ]).map(tab => (
          <button
            key={tab.value}
          ...
          >
            <tab.Icon size={14} className={statusFilter === tab.value ? tab.color : ''} />
            {tab.label}
      ...
      {loading && !bulkUploading ? (
            ...
            <tbody>
              {[...Array(5)].map((_, i) => (
                <tr key={i} className="border-b last:border-0">
                  ...
                  <td className="px-5 py-4"><div className="h-7 w-32 bg-gray-200 rounded animate-pulse ml-auto" /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <CPUsersTable
          ...
          onEdit={user => { setEditUser(user); setShowFormModal(true); }}
