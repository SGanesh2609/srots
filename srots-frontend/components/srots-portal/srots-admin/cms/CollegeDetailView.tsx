...
//             ) : (
//               <img
//                 src={college.logoUrl || '/default-college-logo.png'}
//                 alt={college.name}
...
//         cpCount: college.cphCount || 0,
//         totalJobs: 0,
//         activeJobs: college.activeJobs || 0
//       });
//     } finally {
//       setLoadingStats(false);
//     }
...
import { CollegeService } from '../../../../services/collegeService';
import { ResourceService } from '../../../../services/resourceService';
import { ToggleLeft, Camera, Building, Loader2, ArrowLeft, CreditCard } from 'lucide-react';
import { GlobalStudentDirectory } from '../../../global/StudentDirectory';
import { CPUsersData } from './CPUsersData';
import { ManagingStudentAccounts } from '../../../global/ManagingStudentAccounts';
import { PaymentManagement } from './PaymentManagement'; // ← NEW IMPORT
...
// Extended tab type to include payments
...
export const CollegeDetailView: React.FC<CollegeDetailViewProps> = ({
  ...
  const tabs: { key: CmsTab; label: string }[] = [
    ...
    { key: 'payments',  label: 'Payment Management'        }, // ← NEW TAB
  ];
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border overflow-hidden animate-in fade-in slide-in-from-right-4">
      ...
      {/* Tabs */}
      <div className="flex border-b px-6 bg-white sticky top-0 z-10 overflow-x-auto">
        {tabs.map(tab => (
          <button
            key={tab.key}
          ...
          >
            {tab.key === 'payments' && <CreditCard size={14} />}
            {tab.label}
          </button>
        ))}
      </div>
