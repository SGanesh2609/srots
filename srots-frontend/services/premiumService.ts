...
export interface PremiumAccessStatus {
  ...
  rejectionReason: string | null;
  /**
   * Included in HTTP 402 responses from AuthController.
   * Used by App.tsx to populate premiumUserId state, which is passed
   * to PremiumPaymentPage and sent to POST /public/submit.
...
export const PremiumService = {
  ...
  /**
   * PUBLIC submit — use when the student is on PremiumPaymentPage AFTER a 402
   * login response. They have NO JWT token in localStorage.
   *
   * Calls: POST /api/v1/premium-payments/public/submit
   * SecurityConfig: /api/v1/premium-payments/public/** → permitAll()
   *
   * Sends userId as a multipart field (obtained from the 402 response body,
   * stored in App.tsx premiumUserId state, passed as a prop to PremiumPaymentPage).
   * The backend validates that userId exists and is a STUDENT.
   *
   * The axios interceptor may or may not add an Authorization header depending
   * on whether a stale token exists in localStorage — the backend ignores it
   * for this endpoint since it is permitAll().
   */
  submitPaymentPublic: async (
    userId:        string,
    utrNumber:     string,
    premiumMonths: PremiumMonthsKey,
    amount:        number,
    ...
    const form = new FormData();
    form.append('userId',        userId);        // ← required, identifies the student
    form.append('utrNumber',     utrNumber);
    form.append('premiumMonths', premiumMonths);
    form.append('amount',        String(amount));
    form.append('username',      username);
    ...
    const response = await api.post('/premium-payments/public/submit', form, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return response.data;
  },
  ...
  /**
   * AUTHENTICATED submit — use when student is already logged in (JWT present).
   * Used by the in-app RenewalModal.
   *
   * Calls: POST /api/v1/premium-payments/submit
   * Requires: STUDENT role JWT in Authorization header.
   * Does NOT send userId — backend reads it from @AuthenticationPrincipal.
   *
  ...
  /**
   * Student: check own premium access state (requires JWT).
   * Backend: GET /api/v1/premium-payments/my-status
   */
  checkMyStatus: async (): Promise<PremiumAccessStatus> => {
    const response = await api.get('/premium-payments/my-status');
   ...
   * Backend: POST /api/v1/premium-payments/direct-extension/{studentUserId}
   */
  grantDirectExtension: async (
    studentUserId: string,
    months:        number,
  ): Promise<{ message: string }> => {
    const response = await api.post(
        `/premium-payments/direct-extension/${studentUserId}`, { months });
    return response.data;
  },
